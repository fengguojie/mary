...
public class CloudNodeIoHandler extends IoHandlerAdapter {
	...
	public void sessionClosed(IoSession session) throws Exception {
		...
		DasConnect dasConnect = dasConnectBiz.findOne(session.getId());
		
		if (dasConnect != null ) {
			String dasId =  dasConnect.getDasId();
		...
		dasConnectBiz.deleteBySessionIdAndServerAddress(session.getId(),localAddress);
	...
	public void setDasConnectBiz(DasConnectBiz dasConnectBiz) {
		...
		mongoOps.remove(query, DASRealTime.class);*/
		dasConnectBiz.deleteByServerAddress(localAddress);
    }
}