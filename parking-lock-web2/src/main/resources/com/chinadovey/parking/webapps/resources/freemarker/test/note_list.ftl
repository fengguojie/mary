			...
			<div class="box box box-primary">
				...
				<div class="box-body">
					...
					<table id="myDatatable" data-toggle="table"
						data-url="${request.contextPath!}/test/note/getList" data-show-refresh="true" data-show-toggle="true" data-show-columns="true"
						data-side-pagination="server" data-pagination="true" data-search="true" data-field-text="按照机构名称搜索"  
						data-page-list="[10, 25, 50, 100, 200]">
						<thead>
							<tr>
								<th data-checkbox="true"></th>
								<th data-field="id"  data-align="left"  >笔记ID</th>
								<th data-field="title"  data-align="left"  >笔记标题</th>
								<th data-field="content"  data-align="left" >笔记内容</th>
								<th data-field="userID"  data-align="left" >用户id</th>
								<th data-field="username"  data-align="left" >用户姓名</th>
								<th data-field="videoId"  data-align="left" >视频id</th>
...
function timeFormatter(value, row, index) {
	...
	$(function() {
		//跳转到添加页面
		$('.add').click(function() {
			var href = '${request.contextPath!}/test/note/add';
	...
	window.operateEvents = {
		'click .detail' : function(e, value, row, index) {
			var href = '${request.contextPath!}/test/note/detail?id='
		...
		'click .edit' : function(e, value, row, index) {
			var href = '${request.contextPath!}/test/note/edit?id='
					+ row.id+'&page='+colentpage;
			if (href != "" && href != "#") {
				$("#main-content").zrPageRefresh(href);
			}
	...
	function deleteRecord(id) {
		$.post(
						'${request.contextPath!}/test/note/delete',
						'id=' + id,
						function(e) {
							if (e.result == 'SUCCESS') {
								$('#myModal').modal('hide');
								...
								$('#myDatatable')
										.bootstrapTable(
												'refresh',
												{
													url : '${request.contextPath!}/test/note/getList'
												});
								setTimeout(function() {
									$("#success-info").hide();
								}, 3000);
