					...
					<table id="myDatatable" data-toggle="table"
						...
						<thead>
							<tr>
								...
								<th data-field="gname"  data-align="left"  >商品名字</th>
								<th data-field="price"  data-align="left" >商品价格</th>
								<th data-field="kinds"  data-align="left" >商品种类</th>
...
function timeFormatter(value, row, index) {
	...
	$(function() {
		//跳转到添加页面
		$('.add').click(function() {
			var href = '${request.contextPath!}/test/pp_Goods/add';
	...
	window.operateEvents = {
		'click .detail' : function(e, value, row, index) {
			var href = '${request.contextPath!}/test/pp_Goods/detail?id='
		...
		'click .edit' : function(e, value, row, index) {
			var href = '${request.contextPath!}/test/pp_Goods/edit?id='
					+ row.id+'&page='+colentpage;
			if (href != "" && href != "#") {
				$("#main-content").zrPageRefresh(href);
			}
	...
	function deleteRecord(id) {
		$.post(
						'${request.contextPath!}/test/pp_Goods/delete',
						'id=' + id,
						function(e) {
							if (e.result == 'SUCCESS') {
								$('#myModal').modal('hide');
								...
								$('#myDatatable')
										.bootstrapTable(
												'refresh',
												{
													url : '${request.contextPath!}/test/pp_Goods/getList'
												});
								setTimeout(function() {
									$("#success-info").hide();
								}, 3000);
